    ...
    class RepairSystemPatch
    {
            ...
            [HarmonyArgument(2)] byte amount)
        {
            ...
            if (!Options.MadmateCanFixComms.GetBool() && player.GetCustomRole().IsMadmate() //Madmateがコミュサボを直せる設定がオフ
                ...
                return false;
            if (player.Is(CustomRoles.Sheriff) || player.Is(CustomRoles.Arsonist) || (player.GetCustomRole() is CustomRoles.Jackal or CustomRoles.JSchrodingerCat && !Options.JackalCanUseSabotage.GetBool()))
            {
