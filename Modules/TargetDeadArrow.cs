...
namespace TownOfHostY
{
    ...
    {
        public static HashSet<DeadBody> DeadBodyList = new();
        public static HashSet<byte> SeerList = new();
        ...
        {
            return role == CustomRoles.Detector;
        }

        //死体が生まれた時、発見される側の死体リストに追加
        public static void UpdateDeadBody()
        {
            if (!CustomRoles.Detector.IsEnable()) return;
            ...
            if (SeerList.Count != 0)
            {
                foreach (var seerId in SeerList)
                {
                    ...
                    Logger.Info($"TargetArrowAdd({seerId}, targetBody)", "TargetDeadArrow");
                }
            }
        }
